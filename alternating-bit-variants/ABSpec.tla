------------------------------ MODULE ABSpec --------------------------------
(*
ABSpec.tla from Leslie Lamport's TLA+ Course, Lecture 9, Part 1
...
(* Just a little experiment of mine to confirm that all three of these
ways of computing the next value of the alternating bit behave the
same in TLA+. *)

NextBitVal1(b) == 1 - b
NextBitVal2(b) == (b + 1) % 2
NextBitVal3(b) == IF b = 0 THEN 1 ELSE 0

FunctionEquivalentInv ==
    /\ \A b \in {0, 1}: NextBitVal1(b) = NextBitVal2(b)
    /\ \A b \in {0, 1}: NextBitVal1(b) = NextBitVal3(b)

A == /\ AVar = BVar
     /\ \E d \in Data: AVar' = <<d, NextBitVal1(AVar[2])>>
     /\ BVar' = BVar
